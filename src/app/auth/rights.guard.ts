...
export const rightsGuard: CanActivateFn = (route, _state) => {
  ...
  const menupara: string = route.data?.['menupara'] ?? '';
  ...
  return rightsService.getPermission(cont, view, menupara).pipe(
    map(perm => {
      if (perm.charAt(0) === 'Y') return true;
      const qp: Record<string, string> = { cont, view };
      if (menupara) qp['menupara'] = menupara;
      return router.createUrlTree(['/unauthorized'], { queryParams: qp });
    })
  );
};
