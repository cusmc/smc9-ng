...
if %GRADLE_RESULT% neq 0 (
  echo.
  echo ERROR: gradlew bundleRelease failed. Aborting.
  exit /b %GRADLE_RESULT%
)

if not exist "%OUT_DIR%" mkdir "%OUT_DIR%"
set AAB_NAME=smc9-canteen-v%NEW_VERSION_NAME%-%NEW_VERSION_CODE%.aab
copy /y "android\app\build\outputs\bundle\release\app-release.aab" "%OUT_DIR%\%AAB_NAME%" >nul

echo.
echo Build complete: %OUT_DIR%\%AAB_NAME%
